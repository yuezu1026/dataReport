...
public class ComputingAmazonServiceImpl implements ComputingService {
	...
	if ("1".equals(computingMethod)) {
	    ...
	    //发货国交税
	    List<Map> vatCTypeList = orderAmazonMapper.sumCTypeVatAmountBycurrency(paramMap);
	...
	if (CollectionUtils.isNotEmpty(countryList)) {
	    ...
	    if("GB".equals(needComputingCountry)) {
        		...
        		nonStandardVatRate = country.getInitVatRate();
        	    }else if("low".equals(taxRateType)){
        		nonStandardVatRate = country.getLowVatRate();
        	    }else {
