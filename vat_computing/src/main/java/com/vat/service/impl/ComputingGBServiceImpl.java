...
@Service
public class ComputingGBServiceImpl implements ComputingService {

    private final static Logger logger = LoggerFactory.getLogger(ComputingGBServiceImpl.class);
    ...
    @Override
    public void computing(String userId, String[] periodList, String needComputingCountry,String taxRateType) {
	...
	if (periodList != null && periodList.length > 0) {
		...
		if (StringUtils.isNotBlank(period)) {
		    ...
		    computingResultByPeriod(userId, period, needComputingCountry,taxRateType);
		} else {
	...
	if (CollectionUtils.isNotEmpty(computingResultList)) {
		...
		String privilegeFlag = user.getPrivilegeFlag();
		pdfService.createPDF(uploadRoot, outPath,privilegeFlag);
	    }
	}
	...
	String computingMethod = getComputingMethod(userId);
	if ("1".equals(computingMethod)) {
	    //交易产生地交税
	    List<Map> vatATypeList = orderGBMapper.sumATypeVatAmountBycurrency(paramMap);
		...
		for (Map map : vatATypeList) {
		    ...
		    inputMap.put("dataType", DATA_TYPE);
	    ...
	    List<Map> vatBTypeList = orderGBMapper.sumBTypeVatAmountBycurrency(paramMap);
	    ...
	    // 发货国交税
	    List<Map> vatCTypeList = orderGBMapper.sumCTypeVatAmountBycurrency(paramMap);
	    if (CollectionUtils.isNotEmpty(vatCTypeList)) {
		logger.info("vatList is not null");
		Map<String, String> inputMap = null;
		for (Map map : vatCTypeList) {
		    String currencyCode = (String) map.get("currencyCode");
		    BigDecimal vatAmount = new BigDecimal(MapUtils.getDoubleValue(map, "vatAmount"));
		    logger.info("currencyCode: " + currencyCode);
		    logger.info("vatAmount: " + vatAmount);
    ...
    @Override
    public void computingResultByPeriod(String userId, String period, String needComputingCountry,String taxRateType) {
	...
	String nonStandardVatRate = null;
	...
	if (CollectionUtils.isNotEmpty(countryList)) {
	    ...
	    if ("GB".equals(needComputingCountry)) {
		if ("init".equals(taxRateType)) {
		    standardVatRate = country.getInitVatRate();
		} else if ("low".equals(taxRateType)) {
		    standardVatRate = country.getLowVatRate();
		} else {
	...
	List<Map> middleResultList = middleResultMapper.findMiddleTotalAmountResultList(userId, period,
		needComputingCountry, DATA_TYPE);
	...
	//initValue = initValue.add(new BigDecimal(vatRate));
	...
	BigDecimal initValue = new BigDecimal(1);
	if(StringUtils.isNotBlank(standardVatRate)) {
        	initValue = initValue.add(new BigDecimal(standardVatRate));
        	vatAmount = includingTaxAmount.divide(initValue, 10, BigDecimal.ROUND_HALF_DOWN)
        		.multiply(new BigDecimal(standardVatRate));
	}else {
	    vatAmount = includingTaxAmount.multiply(new BigDecimal(nonStandardVatRate));
	}
    ...
    private String getComputingMethod(String userId) {
	...
	String computingMethod = "1";
	if (CollectionUtils.isNotEmpty(configList)) {
	    computingMethod = configList.get(0).getComputingMethod();
	}
	...
	for (String period : periodList) {
	    computingResultList = computingResultMapper.findComputingResult(userId, period, needComputingCountry,
		    DATA_TYPE);
	    if (CollectionUtils.isNotEmpty(computingResultList)) {
	...
	for (String period : periodList) {
	    computingResultList = middleResultMapper.findMiddleResultList(userId, period, needComputingCountry,
		    DATA_TYPE);
	    if (CollectionUtils.isNotEmpty(computingResultList)) {
		reCcomputingResultList.addAll(computingResultList);
	    }
