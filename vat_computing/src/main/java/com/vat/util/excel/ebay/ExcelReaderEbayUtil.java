 ...
 * @desc
 **/
public class ExcelReaderEbayUtil {

    private final static Logger logger = LoggerFactory.getLogger(ExcelReaderEbayUtil.class);
    ...
    public static List<EbayOrder> readSaveSingleSheetData(IOrderEbayDAO orderDAO, String userId,
	    Map<String, Integer> colNameMap, String sheetName, List<String> cellList) {
	List<EbayOrder> orderList = new ArrayList<EbayOrder>();

	EbayOrder order = null;
	...
	order = new EbayOrder();
	order.setEbay_order_id(UUIDUtils.getUUID());
    ...
    public static int readExcel(String filePath, String userId, IOrderEbayDAO orderDAO) throws Exception {
	int totalRows = 0;
	if (filePath.endsWith(EXCEL03_EXTENSION)) { // 处理excel2003文件
	    ExcelXlsReaderEbay excelXlsReader = new ExcelXlsReaderEbay(userId, orderDAO);
	    totalRows = excelXlsReader.process(filePath);

	    List<EbayOrder> batchOrderList = excelXlsReader.getBatchOrderList();
	    ...
	    ExcelXlsxReaderEbay excelXlsxReader = new ExcelXlsxReaderEbay(userId, orderDAO);
	    totalRows = excelXlsxReader.process(filePath);

	    List<EbayOrder> batchOrderList = excelXlsxReader.getBatchOrderList();
	    if (CollectionUtils.isNotEmpty(batchOrderList)) {
		orderDAO.batchOrderSave(batchOrderList);
	    }
	...
	ExcelReaderEbayUtil.readExcel(path, null, null);
    }
}