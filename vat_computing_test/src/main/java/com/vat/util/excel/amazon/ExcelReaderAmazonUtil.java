...
public class ExcelReaderAmazonUtil {
    ...
    public static List<Order> readSaveSingleSheetData(IOrderAmazonDAO orderDAO, String userId,
	...
	if (filePath.endsWith(EXCEL03_EXTENSION)) { // 处理excel2003文件
	    ...
	    List<Order> batchOrderList = excelXlsReader.getBatchOrderList();
	    ...
	    List<Order> batchOrderList = excelXlsxReader.getBatchOrderList();
	    if (CollectionUtils.isNotEmpty(batchOrderList)) {
		orderDAO.batchOrderSave(batchOrderList);
	    }
